...
struc globals
	...
	.per_node_dict: resb node_struct_size
	.per_node_ftar: resb node_struct_size
...
%ifndef SNAPSHOT_MODE
	...
	call falktp_pull
	mov  rbx, [fs:globals.fs_base]
	lea  rbx, [rbx + globals.per_node_dict]
	call init_node_data
	...
	call falktp_pull
	mov  rbx, [fs:globals.fs_base]
	lea  rbx, [rbx + globals.per_node_ftar]
	call init_node_data

	; Get the snapshot
	mov  r13, 0x8000000001230000
	call falktp_pull
	mov  [fs:globals.snapshot0], rsi
%endif ; SNAPSHOT_MODE
...
%if 1
	...
	; Hook 1 - UfsIoCache::Read on success path
	; mpengine+0x9f81
	mov rbx, [rax + VMCB.rip]
	mov dr0, rbx
	mov rbx, (1 << 0)
	mov qword [rax + VMCB.dr7], rbx

	; End on ret
	mov rdx, [rax + VMCB.rip]
	sub rdx, 0x9f81
	add rdx, 0x6AC3
...
%endif
	...
	call select_dict_ext
	call corrupt_quantum
	call inject_corrupt_bytes
...
%endif
	...
	; Get virtual RIP of this access
	mov ecx, 0xc0011034
	rdmsr
	shl  rdx, 32
	or   rdx, rax
	call mm_read_guest_qword
	cmp  dl, 0x80 ; cmp r/m8, imm8
	je  .cmp_inst
	cmp  dl, 0x38 ; cmp r/m8, reg8
	je  .cmp_inst
	cmp  dl, 0x3a ; cmp reg8, r/m8
	je  .cmp_inst
	jmp .not_ibs
...
.cmp_inst:
	...
	sub rsp, access_report_size

	sub rax, rdi
	mov rdi, [gs:thread_local.VMCB]
	mov rdi, [rdi + VMCB.br_to]

	mov dword [rsp + access_report.magic], 0x2eb081e7
	mov qword [rsp + access_report.offs],  rax
	mov qword [rsp + access_report.rip],   rdi

	mov  rbx, rsp
	mov  rcx, access_report_size
	call x540_send_packet

	add rsp, access_report_size
